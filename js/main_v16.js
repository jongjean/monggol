...
function initThreeJS() {
  ...
  // 공용 지오메트리 생성 (메모리 최적화)
  const sharedGeometry = new THREE.PlaneGeometry(3.0, 2.0);
  ...
  for (let i = 0; i < 32; i++) {
    ...
    loader.load(imagePath, (texture) => {
      // const imgGeometry = new THREE.PlaneGeometry(3.0, 2.0); // 공용 지오메트리 사용
      const imgMaterial = new THREE.MeshBasicMaterial({ map: texture });
      const imgMesh = new THREE.Mesh(sharedGeometry, imgMaterial);
    ...
    // 스피커 아이콘 추가 (012, 005, 009, 025, 019번 작품)
    const speakerArtworks = ["012", "005", "009", "025", "019"];
    if (speakerArtworks.includes(imgNum)) {
      const sprite = createSpeakerIcon();
  ...
  function createSpeakerIcon() {
    ...
    sprite.position.set(0, 1.3, 0); // 액자 위쪽
  ...
  // 실제 초기 카메라 위치 (-22.99, 21.27, -0.40)에서 각도와 반경 계산
  const initialX = -22.99;
  const initialY = 21.27;
  const initialZ = -0.40;
  const centerY = 2.5; // lookAt 기준점 (작품 높이와 일치)

  // 반경 계산: sqrt(x² + (y-centerY)² + z²)
  let cameraRadius = Math.sqrt(initialX * initialX + (initialY - centerY) * (initialY - centerY) + initialZ * initialZ);

  // 수평 각도 계산: atan2(x, z)
  let cameraAngleH = Math.atan2(initialX, initialZ);

  // 수직 각도 계산: asin((y-centerY) / radius)
  ...
  canvas.addEventListener('click', (event) => {
    ...
    if (intersects.length > 0 && isDoubleClick) {
      ...
      const artworkId = clicked.userData.artworkId;
      ...
      // 도슨트 팝업도 열기 (artworkId가 있는 경우) - 이미지 팝업 후에
      if (artworkId && window.openDoc) {
        setTimeout(() => window.openDoc(artworkId), 100);
      }
    }
  });
  ...
  function animate() {
    ...
    window.dispatchEvent(new Event('furgonIconAnimate')); // 푸르공 아이콘 애니메이션
...
(function () {
  const data = {
    '012': {
      title: '한 때 한 곳을 스쳐간 사계', description: `검푸르게 화창하던 하늘에서 산을 하나 넘어서자
갑자기 눈보라가 휘몰아쳤다.
바람이 너무도 세 눈이 수평으로 내렸다.
흠칫 순간 무엇인가 지나가는 가 했더니
눈보라 속을 무서운 속도로 질주하는
...
window.addEventListener('load', function () {
  setTimeout(() => {
    ...
    // 크롬 모바일 감지
    const isChromeMobile = /Chrome/.test(navigator.userAgent) && /Android/.test(navigator.userAgent);
    ...
    canvas.addEventListener('click', function (e) {
          ...
          for (let i = 0; i < hits.length; i++) {
            ...
            while (obj) {
              ...
              if (obj.userData && obj.userData.artworkId) {
                console.log('🎨 작품 터치:', obj.userData.artworkId);

                // 도슨트 팝업 열기
                window.openDoc(obj.userData.artworkId);

                // 작품 이미지도 팝업으로 열기
                // artworkIndex는 부모 frameGroup에 있을 수 있음
                let artworkIndex = obj.userData.artworkIndex;
                if (!artworkIndex && obj.parent && obj.parent.userData) {
                  artworkIndex = obj.parent.userData.artworkIndex;
                }
                if (artworkIndex) {
                  const imagePath = `images/artworks/${String(artworkIndex).padStart(3, '0')}.jpg`;
                  ...
                  img.onclick = (e) => {
                    ...
                    if (now - imgLastClick < 300) {
                      document.body.removeChild(popup);
                      if (window.closeArtwork) window.closeArtwork();
                    }
                    imgLastClick = now;
                  };
                }
      ...
      } else if (isChromeMobile && now - lastClick > 300) {
        // 크롬 모바일: 싱글 클릭으로도 푸르공 감지 (작품은 더블클릭만)
        const mouse = {
          x: (e.clientX / window.innerWidth) * 2 - 1,
          y: -(e.clientY / window.innerHeight) * 2 + 1
        };
    ...
    canvas.addEventListener('touchend', function (e) {
      ...
      if (e.changedTouches[0]) {
        ...
        // 더블탭: 500ms 이내, 같은 위치 (100px 이내)
        if (timeDiff > 50 && timeDiff < 500 && distX < 100 && distY < 100) {
          e.preventDefault();

          console.log('📱 모바일 더블탭 감지!', navigator.userAgent);
          ...
          if (window.raycaster && window.camera && window.scene) {
            ...
            console.log('Raycasting hits:', hits.length);
            ...
            for (let i = 0; i < hits.length; i++) {
              ...
              while (obj) {
                ...
                if (artworkIndex) {
                  console.log('🎨 작품 터치:', artworkIndex);
                  const artworkId = obj.userData.artworkId || (obj.parent && obj.parent.userData && obj.parent.userData.artworkId);
                  const imagePath = `images/artworks/${String(artworkIndex).padStart(3, '0')}.jpg`;
                  const popup = document.createElement('div');
                  popup.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.5);z-index:10000;display:flex;align-items:center;justify-content:center;';
                  const img = document.createElement('img');
...
// 푸르공 관련 코드는 furgon_system.js에서 관리 (중복 제거)
